...
|HOMEBREW_TAP_TOKEN | Personal Access Token (PAT) in Resource jococo-ch/homebrew-atp with repo scope and Read & Write permissions for Content | PAT | release.yml
|RELEASE_PLZ_TOKEN | Personal Access Token (PAT) in Resource jococo-ch, repository access to this repo and Read & Write permissions for Content and PRs | PAT | release-plz-pr.yml and release-plz.yml
|===
...
| Release-plz release | release-plz.yml | release-plz creating a tag, amending changelog, creating release notes.
| Release-plz Create Release PR | release-plz-pr.yml | release-plz using semver to create a PR to release a new version.
| Release | release.yml | cargo-dest creating the released build artifacts, attaching them to the github release
|===
...
Regenerate the  Personal Access Token (PAT) lola-sumup-release-plz https://github.com/settings/personal-access-tokens[here].
Go https://github.com/jococo-ch/lola-sumup/settings/secrets/actions[here] to paste the new token to the Repository secert RELEASE_PLZ_TOKEN.
